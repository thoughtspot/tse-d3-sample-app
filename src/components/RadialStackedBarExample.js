...
import { RadialStackedBarChart } from './util/d3-helpers.js';
...
export default function RadialStackedBarExample(props) {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            var estatesMap = new Map(); // Item type sales by estate
            var columnsMap = new Map(); // All different item types

            newData.forEach(function(item, index) {
                if (!estatesMap.has(item[0])) {
                    estatesMap.set(item[0], []);
                }
                if (!columnsMap.has(item[1])) {
                    columnsMap.set(item[1], item[1]);
                }
                estatesMap.get(item[0]).push({
                    "item": item[1],
                    "subtotal": item[2]
                });
            });
            ...
            for (const [key, value] of estatesMap.entries()) {
                ...
                items.forEach(function(item, index) {
                    chartRow[item['item']] = item['subtotal'];
                    total += item['subtotal'];
                });
